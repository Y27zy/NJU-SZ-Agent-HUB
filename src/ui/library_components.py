...
export default function(component){
 ...
 (parentElement.closest?.('.stApp')||parentElement.getRootNode()?.host?.closest?.('.stApp'))?.classList.remove('workspace-component-ready');
 const cats=data?.categories||[],docs=data?.documents||[],active=data?.active||'custom',jobs=docs.map(d=>d.job).filter(Boolean);
 const signature=JSON.stringify({active,docs:docs.map(d=>[d.id,d.title,d.status,d.pages,d.can_delete,d.can_reprocess,d.job&&[d.job.id,d.job.status,d.job.progress_message,d.job.attempt_count,d.job.next_retry_at]]),cats:cats.map(c=>[c.key,c.count]),canAdd:!!data?.canAdd});
 ...
 ${docs.length?`<div class="doc-list">${docs.map(d=>{const job=d.job,busy=!!job,busyText=job?.progress_message||(job?.status==='queued'?'等待后台处理':'后台整理中');return `<article class="doc-row"><div><div class="doc-title">${d.title}</div><div class="doc-meta">${d.kind} · ${d.global?'全局同步':'仅自己可见'} · ${busy?busyText:d.status}${job?.attempt_count?` · 已自动续跑 ${job.attempt_count} 次`:''}${d.pages?` · ${d.pages} 页`:''}</div></div><div class="doc-actions">${busy?`<button class="cancel" data-cancel-job="${job.id}">取消整理</button>`:(d.can_reprocess?`<button data-reprocess="${d.id}" title="使用当前模型重新识别章节和排版">重新整理</button>`:'')}${d.can_delete&&!busy?`<button class="danger" data-delete="${d.id}">删除</button>`:''}<button data-open="${d.id}" class="study">学习</button></div></article>`}).join('')}</div>`:`<div class="empty-library"><strong>这里还没有资料</strong><span>${data?.canAdd?'点击“构建新资料”，把 PDF、PPTX 或 Markdown 整理成可交互原文。':'这里由管理员维护，暂时没有公开资料。'}</span></div>`}</section></div>`;
